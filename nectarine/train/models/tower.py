...
class QueryModel(Tower):
    def __init__(
        ...
        super().__init__(n_dims, embedding_dim, feature_layers, output_layers)
        self._session_layers = session_layers or [...]
        self.session_model = self._create_model(
            layer=tf.keras.layers.LSTM, layer_sizes=self._session_layers
        )

    def call(self, inputs, training):
        embedding = self.embedding_layer(inputs["id"])
        features = self.feature_model(inputs["features"])
        session = self.session_model(inputs["session"])
        x = tf.concat([embedding, features, session])
        x = self.output_model(x)
        return x
